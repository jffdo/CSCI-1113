...
def invert(img_matrix):
    ...
    '''
    newMatrix = copy.deepcopy(img_matrix)
    for row in range(len(img_matrix)):
        for column in range(len(img_matrix[row])):
            for rgb in range(len(img_matrix[row][column])):
                newMatrix[row][column][rgb] = 255-img_matrix[row][column][rgb]
    return newMatrix
...
def grayscale(img_matrix):
    ...
    for row in range(len(img_matrix)):
        for column in range(len(img_matrix[row])):
            r = img_matrix[row][column][0]
            g = img_matrix[row][column][1]
            b = img_matrix[row][column][2]
            avg = (r+g+b)/3
            for rgb in range(len(img_matrix[row][column])):
                newMatrix[row][column][rgb] = int(avg)
    return newMatrix
...
def edge_detect(img_matrix):
    ...
    for row in range(len(img_matrix)):
        for column in range(len(img_matrix[row])):
            if row == 0 or row == len(img_matrix)-1 or column == 0 or column == len(img_matrix[row])-1:
                for rgb in range(len(img_matrix[row][column])):
                    newMatrix[row][column][rgb] = 0
            else:
                for rgb in range(len(img_matrix[row][column])):
                    val = (img_matrix[row][column+1][rgb] + img_matrix[row][column-1][rgb] + img_matrix[row+1][column][rgb] + img_matrix[row-1][column][rgb])/4
                    if img_matrix[row][column][rgb] >= (val+10):
                        newMatrix[row][column][rgb] = 255
                    else:
                        newMatrix[row][column][rgb] = 0
    return newMatrix
